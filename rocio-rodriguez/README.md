...
```text
1. Es un metodo utilizado para entrenar modelos en base a un volumen de datos, donde deberá aprender patrones de los mismos
2. El dato que se busca obtener o predecir
3. Es la variable objetivo que indica si una venta fue alta o no alta
4. 'X' representa las variables de entrada, mientras 'y' representa la variable objetivo
5. Es la división de los datos en categorías o clases
6. Porque se busca predecir si una venta entra en una categoría, donde solo existen dos posibles caminos
7. Crea un arbol de decisiones para entrenar el modelo
8. Para dividir los datos en dos, una parte para permitir al modelo entrenar y aprender patrones, y una segunda para poder aplicar los patrones y evaluar su exactitud
9. Es la proporción en que un modelo puede acertar o fallar sus predicciones
10. Para visualizar de mejor manera como el modelo predijo y ver en que se equivocó de una forma más clara
11. Es un método que da inicio al proceso de entrenamiento del modelo
12. Método para que el modelo comience las predicciones de los datos dados
13. Porque permite pasar columnas categóricas numéricas para que el modelo pueda trabajar con esos valores
14. Porque obliga a alinear las columnas a trabajar con las que utiliza el modelo
15. Porque el modelo siempre tiene un cierto porcentaje de equivocarse
```
...
Coincidencias: 9
Errores: 1
Conclusion breve: El modelo pareción entrenar correctamente y enlazar patrones en los datos, sin embargo al utilizarse en un dataset con nuevos datos falló en al menos una de las ventas, lo que es normal en un modelo pues siempre existe un margen de error. El hecho de que tenga un buen nivel de exactitud no garantiza que nunca se equivoque, solo que hay menos probabilidad de esto.
```
