...
```text
1. Es cuando una algoritmo es entrenado para aprender a predecir o clasificar un numero de datos y generar datos nuevos en base a lo aprendido.

2. Es el resultado de un modelo en el que se realizo una prediccion en base a algunos datos.

3. Significa cuando cierto numero de datos cumplen con una condicion en vase a una condicion de una venta alta.

4. La diferencia es que en la variable X son datos de entrada y la variable son datos de salida es decir es un resultado que se tiene esperado.

5. Podriamos decir que es cuando una columna es capaza de aprender en base a la conversion de numeros para que el modelo pueda aprender mejor y al agregar columnas nuevas no tenga problemas en aprender con ello.

6. Porque al no convertir las columnas en numeros el modelo no seria capaz de aprender si se llegara a agregar nuevos datos y por ende generaria errores.

7. Permite tomar decisiones como un arbol de predicciones para clasificar los datos que el modelo aprendio.
 
8. Para separar los datos clasificados que se tomaron en cuenta y los resultados que no al momento de realizar pruebas con el entrenamiento, es decir separa lo que nos dio en base a un resultado y conservar los que ya tenemos.

9. Sigfica cuando los datos son similares o se esperan a los resultados del modelo aprendido.

10. Sirve para separar lo que vendria siendo los datos que se usaron para la prueba, es decir los resultados. En este caso cuantos datos fueron acertados y cuales no.

11. Significa que puede filtrar los datos en base a una condicion dada para clasificar los datos.

12. Sirve para que podamos guardar el modelo en base a lo aprendido, es decir lo que ya se predijo anteriormente para que podamos reutilizar el modelo.

13. Sirve convertir las columnas en valores numericos para que el modelo se le sea mas facil aprender de ellas y al agregar nuevas no tenga problema con ello.

14. Sirve para que podamos reutilizar lo que vendria siendo modelos con los que se aprendio anteriormente un patron, reutilizar columnas, filas, etc, sin necesidad de volver a empezar desde cero.

15. Porque solo sirve como una estimacion para un posible resultado y mas si se tiene un menor numero de datos para predecir algo.
```
...
```text
Nombre: Elias Manuel Marquez Bailon
Grupo: 9°A
Materia: Extraccion de conocimiento de bases de datos
Exactitud obtenida: 1.0
Ventas nuevas evaluadas: 70
Coincidencias: 12
Errores: 0
Conclusion breve: Se pudo hacer con exito las predicciones tanto las actuales como nuevas
```
