...
```text
1. Machine learning ocupando datos que conocemos
2. La variable la cual nosotros queremos ocupar como métrica para el modelo
3. Una medida que nosotros ocupamos para definir si una venta es mayor o igual a 1000 (valor: true) o no (valor: false)
4. X son las variables de entrada. Y es nuestra variable objetivo, la que nos ayuda a predecir
5. 
6. 
7. crear un árbol de decisiones 
8. no sé
9. que parte de los valores resultaorn correctos en base del total de valores
10. para obtener cuantos valores son correctos y cuantos no (falsos negativos, falsos positivos, correctos)
11. entrenar el modelo
12. La función para predecir valores usando X_test
13. Para transformar de texto a numeros (valores 0 y 1 para que las computadoras lo entiendad y lo manejen)
14. Para que se recalcule el índice 
15. Porque es una simple estimación, no es un hecho.
```
...
Coincidencias: 8
Errores: 2 
Conclusion breve: El modelo en general funciona correctamente y predijo en un principio el 100% de los valores correctamente, el problema ocurre despues que agregamos más datos de prueba de ventas y como se puede observar, ya no resultó lo mismo y encontramos 8 coincidencias y 2 errores, a pesar de obtener una exactitud excelente, recordemos que las predicciones son solo estimaciones y no hechos veridicos. 
```
