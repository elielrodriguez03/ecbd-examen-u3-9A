...
```text
1.Es un algoritmo de inteligencia artificial que permite realizar predicciones de ciertosparametros para preder si los mismos ocurriran de acuerdo a los valores de entrada x con la solucion propuesta de yde esta manera con datos ya dentro del dataFrame.

2.La variable objetivo o y es la variable de la cual queremos obtener una prediccion es decires la varaible que queremos predecir.

3. venta alta por su clasificacion en 1 y 0 nos permite clasificar de acuerdo a los parametros establecidos en la lambda lo caul nos permite clasificar los mismso para su posterior estudio con el modelo y tambien Representa si el total de una venta fue mayor o igual a 1000 (1) o menor a 1000 (0).

4.Las variables de entrada son los datos que usamos para predecir siendo x,  mientras que la variable objetivo es el resultado que queremos adivinar siendo y.

5.Es un problema de aprendizaje supervisado donde el objetivo es predecir a qué categoría o clase pertenece un dato. 

6. Porque buscamos predecir una categoría específica (venta alta o no), no un valor numérico continuo.

7.Es un algoritmo de clasificación que crea un modelo en forma de árbol, dividiendo los datos con base en reglas hasta llegar a una clasificación.

8.Para dividir los datos en dos partes: una para entrenar el modelo (entrenamiento) y otra para probar su rendimiento (prueba).

9.Es una métrica que indica la proporción de predicciones correctas sobre el total de predicciones realizadas.

10. Sirve para visualizar el desempeño del clasificador, mostrando exactamente dónde acertó y dónde se equivocó (falsos positivos, verdaderos positivos, etc).

11.Es el método que ejecuta el entrenamiento, ajustando el modelo a los datos de entrada y sus respuestas.

12.Toma un conjunto de datos nuevos y utiliza lo aprendido para generar predicciones.

13.Convierte variables de texto (categóricas) en columnas numéricas (0 y 1), permitiendo que los modelos matemáticos las procesen.

14.Para alinear las columnas de los datos nuevos con las columnas exactas que se usaron en el entrenamiento del modelo.

15.Porque los modelos son probabilísticos y pueden fallar, no son perfectos y cometen errores.
```
