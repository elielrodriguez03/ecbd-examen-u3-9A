...
```text
1. Es cuando el modelo aprende con datos que ya traen la respuesta correcta, para después predecir en datos nuevos.
2. Es la variable que quiero predecir.
3. Dice si una venta fue alta (1) o no (0), según si pasó de 1000.
4. X son los datos o variables de entrada que uso para predecir, y es lo que quiero predecir.
5. Es cuando el modelo predice una categoría, no un número.
6. Porque venta_alta solo tiene dos valores: 0 o 1.
7. Hace un árbol de decisiones con reglas tipo "si pasa esto, entonces puede pasar".
8. Para probar el modelo con datos que no vio antes y ver si de verdad aprendió.
9. Es el porcentaje de veces que el modelo acertó.
10. Muestra en qué se equivocó el modelo, no solo cuánto.
11. Es entrenar el modelo con los datos de entrenamiento.
12. Es cuando el modelo ya entrenado hace predicciones.
13. Porque el modelo no entiende texto, solo números.
14. Porque a veces get_dummies genera columnas distintas en los datos nuevos, y reindex las deja igual que en el entrenamiento.
15. Porque el modelo puede fallar, sobre todo en casos que están muy cerca del límite.
```
