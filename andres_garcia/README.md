...
Sirve para evaluar los resultados obtenidos en las predicciones del modelo, sirve para validar cosas como Falsos Positovos, Falsos Negativos, Positivos Reales, etc.

### Que significa `fit()`?
Es la funcion para cargar la data al modelo y entrenarlo

### Que significa `predict()`?
Es la funcion para obtener las predicciones del modelo

### Por que se usa `pd.get_dummies()`?
Se usa para convertir los datos de texto a numerico de forma que el modelo pueda comprenderlos y aprender.

### Por que se usa `reindex()` al predecir ventas nuevas?
Para evitar que haya errores con las columnas y el modelo no tenga problemas prediciendo

### Por que una prediccion no es una verdad absoluta?
Porque solo es un seguimiento de patrones que genera el modelo, entre mayor cantidad de datos hayan sido usado para entrenarlos sera mas preciso, pero no deja de existir un rango de incertidumbre ya que basicamente adivina.
